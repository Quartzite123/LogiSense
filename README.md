...
> A production **FastAPI + React** logistics analytics dashboard built for
> Kiirus Xpress Pvt. Ltd. (Aurangabad-based logistics aggregator, channel partner
> for Delhivery and Blue Dart), with an integrated **AI assistant** (Groq / Llama 3.3 70B).
>
...
The founders use this to monitor SLA compliance, catch at-risk orders before breach,
...
|---|---|---|
| Data pipeline | Python 3.x (pandas, openpyxl) | Untouched from Streamlit era — battle-tested |
...
│   ├── sections/                 # Legacy Streamlit UI (reference only, not served)
│   ├── components/               # Legacy Streamlit components (reference only)
│   ├── reference/
...
│       └── assistant.py          # POST /api/assistant/chat (Groq)
│
├── frontend/                     # React SPA
...
├── LogiSense_sample_data.xlsx    # 1,000-row synthetic demo data
├── requirements.txt              # Python deps (streamlit legacy + fastapi stack)
├── streamlit_app.py              # ⚠ Legacy entry point (deprecated, kept for reference)
└── stlite/                       # ⚠ Legacy WASM bundle (deprecated)
```
...
data and must never change without founder sign-off.
...
- **Exception:** `/api/aggregate-transit/company-detail` uses the original 4-bucket
  ...
  are bucketed by date — this matches the original Streamlit layout exactly.

### 4.8 The `client` vs `order_id` Quirk (IMPORTANT)

In real Delhivery exports for aggregators:
- `client` column = the Delhivery account name (constant, e.g. `kiirus b2bc`) — useless for grouping
- `order_id` column = the actual **end-client company name** (KENSTAR, DTPL, …)
...
| 2 | Order id | order_id | **Real company name** for aggregators (not `client` — see §14 quirk 5) |
| 3 | No of boxes | no_of_boxes | Integer |
| 4 | Client | client | Delhivery account name — constant `kiirus b2bc` for aggregators, useless for grouping |
...
| 10 | Origin City | origin_city | Almost always "Aurangabad" for Kiirus |
...
| POST | `/api/assistant/chat` | `{message, history[]}` → `{reply, context_rows}` (see §9) |
...
no toolbars/zoom (founders found them confusing) · company labels truncate at
14 chars + "…", rotate -40°, 120px reserved height, full name in tooltip.
...
| 6 indexes on hot columns | `main.py` startup | Fast GROUP BY / WHERE |
...
cd C:\Users\Msi-Laptop\Desktop\LogiSense\backend
python -m uvicorn main:app --reload --port 8000

# Window 2 — frontend
cd C:\Users\Msi-Laptop\Desktop\LogiSense\frontend
npm run dev
```
...
7. **Sample data ODA mismatch** — `LogiSense_sample_data.xlsx` currently contains
   invented "ODA" pincodes that don't exist as ODA in the master → they resolve
   to NO/UNKNOWN. Real Delhivery data works correctly. (Regeneration pending — §15.)

8. **The empty-ODA chart fix** — `/api/tat/oda-chart` omits a group with zero rows
   entirely, so the frontend never renders a phantom empty ODA slot (a visual bug
   in the original Streamlit version's chart).
...
10. **Two risk classifiers exist deliberately** — the shared `transit_risk.py`
    ...
    classification inside aggregate-transit's company-detail (matches Streamlit).

11. **Legacy Streamlit still in repo** — `streamlit_app.py`, `app/sections/`,
    `app/components/`, `stlite/` are deprecated but kept as the reference
    implementation for business logic and UI parity checks. Do not delete without
    founder sign-off.

12. **`.env` must never be committed** — contains the Groq API key. Verify
...
| Item | Priority | Notes |
|---|---|---|
| **Electron desktop .exe** | High | Wrap FastAPI + React build into a native Windows app. Final phase. |
| Regenerate sample data with real ODA pincodes | Medium | Query `pincode_master_live WHERE oda='YES'`, use only verified pincodes |
| "TAT Already Breached" KPI on Aggregate Transit | Low | Total At Risk + Due Today across companies |
| Mobile CSS media query | Low | `@media (max-width: 768px)` padding adjustments |
...
```
v1  Streamlit dashboard (Kiirus Xpress production)     — original build
v2  stlite/PWA mobile bundle                            — WASM experiment (deprecated)
v3  Rebrand → LogiSense                                 — 98 replacements / 27 files
v4  FastAPI + React migration                           — current architecture
     Phase 1: scaffold + Landing (12 KPI cards)
     Phase 2: all sections + shared components + exports
     UI parity pass: charts, tables, filters, modals (UI_DESIGN_SPEC.md)
     Perf pass: caching, indexes, lazy loading
     Phase 3: AI Assistant (Groq / Llama 3.3 70B)
v5  (next) Electron desktop packaging
```

**Why the migration happened:** Streamlit re-runs the entire script on every
interaction — fragile custom CSS/iframe hacks, slow loads, session state lost on
reload, stlite/Pyodide failing on low-RAM phones. FastAPI + React gives instant
navigation, persistent views, a real API layer for the AI assistant, and a clean
path to desktop packaging. All business logic survived the migration untouched —
the 28 tests that passed on day one still pass today.
...
| **LogiSense AI** | The Groq-powered chat assistant with live DB context. |

---

*For the original Streamlit-era documentation see the legacy `PROJECT_CONTEXT.md`
and `CHANGES.md`. This README supersedes both for the current architecture.*
